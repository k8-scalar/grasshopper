        ...
        if data['kind'] == 'NetworkPolicy':
                ...
                for ing in data['spec']['ingress']:
                    ...
                    if 'ports' in ing:
                        for p in ing['ports']:
                            ...
                            if 'protocol' in p and not 'port' in p:
                                ports = [p['protocol'], None, None]
                                all_ports.append(ports)
                    new_policy = Policy(data['metadata']['name'], PolicySelect(select), allow, PolicyIngress, all_ports, cidr)
            ...
            if 'Egress' in data['spec']['policyTypes']:
                for eg in data['spec']['egress']:
                    ...
                    new_policy = Policy(data['metadata']['name'], PolicySelect(select), allow, PolicyEgress, all_ports,cidr)
