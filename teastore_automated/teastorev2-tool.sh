...
kubectl -n teastore label po $(kubectl get pods -n teastore | grep teastore-locust | awk '{print $1}') app=teastore
...
kubectl apply -f teastore-policy2.yml 
...
then
  helm delete teastore -n teastore;
  kubectl delete networkpolicies --all -n teastore;
  kubectl delete hpa --all -n teastore;
  kubectl delete po --all -n teastore;
  kubectl delete ns teastore;
  cd;
  rm  ~/csvdocs/teastore*;
  sleep 2
