...
def policy_shadow(containers: List[Container], policies: List[Policy]) -> List[Tuple[int, int]]:
    shad_pols=[]
    cont_sel_pols = []
...
def policy_conflict( containers: List[Container], policies: List[Policy]) -> List[Tuple[int, int]]:
    """
    Policy conflict.
...
'''
#initially had defined all permissive as selecting all pods in the ns
#but this can be misleading as sometimes there a few pods in the ns and can all be genuinely selected
    ...
    for i, policy in enumerate(policies):
        ...
        for idx, cont_info in enumerate (containers):#in range(n_container):
            ...
            if allow_set[idx] and not policy.allow_policy(containers[idx]):
                allow_set[idx] = False
        if all(select_set) or all(allow_set): #If select set or allow set is all True
            perm_pols.append(i)

    return perm_pols
'''
def over_permissive(policies: List[Policy]) -> List[int]:
    perm_pols=[]
    for i, policy in enumerate(policies):
        if not policy.selector.labels:
            perm_pols.append(policy.name)
    return perm_pols