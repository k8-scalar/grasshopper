        ...
        if data['kind'] == 'NetworkPolicy':
                ...
                for ing in data['spec']['ingress']:
                    ...
                    for f in ing['from']:
                        if 'podSelector' in f:
                            ...
                            #allow.append(vars(allow_labels))
                        ...
                        for p in ing['ports']:
                            if 'protocol' in p:
                                if 'port' in p:
                                    ...
                                    ##ports_protocol=p['protocol']
                                    ##ports_port=p['port']


                    new_policy = Policy(data['metadata']['name'], PolicySelect(select), allow, PolicyIngress, ports, cidr)
                    #new_policy = Policy(data['metadata']['name'], PolicySelect(select), PolicyAllow(allow), PolicyIngress, ports)
                    ##new_policy = Policy(data['metadata']['name']+'-ingress', PolicySelect(select), PolicyAllow(allow), PolicyIngress, ports)
                    ##new_policy = Policy(data['metadata']['name']+'-ingress', PolicySelect(select), PolicyAllow(allow), PolicyIngress, ports_protocol, ports_port)
            ...
            if 'Egress' in data['spec']['policyTypes']:
                for eg in data['spec']['egress']:
                    ...
                    #new_policy = Policy(data['metadata']['name'], PolicySelect(select), PolicyAllow(allow), PolicyEgress, ports)
                    ##new_policy = Policy(data['metadata']['name']+'-egress', PolicySelect(select), PolicyAllow(allow), PolicyEgress, ports)
                    ##new_policy = Policy(data['metadata']['name']+'-egress', PolicySelect(select), PolicyAllow(allow), PolicyIngress, ports_protocol, ports_port)
                    self.policies.append(new_policy)
