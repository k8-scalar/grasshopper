...
def on_created(event):
    ...
    if event.is_directory:
        ...
        offenders = {'redundant':[], 'conflict':[], 'permissive':[]}
        cp = ConfigParser('data/')
        ...
        redund= policy_shadow(containers,policies )
        for item in redund:
            offenders['redundant'].append(item)
        conf= policy_conflict(containers, policies)
        for item in conf:
            offenders['conflict'].append(item)
        perm_pols = over_permissive(policies)
        for item in perm_pols:
            offenders['permissive'].append(item)
        if pol_name in redund or pol_name in conf or pol_name in perm_pols:
            print('offenders: ', offenders)

        for v in policies:
            if v.name not in redund and v.name not in conf and v.name not in perm_pols:
                ...
                print("{} is in offenders".format(v.name))
    elif kind == 'Service':
