...
def main():
    ...
    3. <namespace>: A string indicating the Kubernetes namespace to watch (e.g., "default", "test").
    ...
    Usage:
        gh.py <singleSGPerNodeScenario> <distributed> <namespace>
    """
    if len(sys.argv) != 4:
        print("Usage: gh.py <singleSGPerNodeScenario> <distributed> <namespace>")
        sys.exit(1)

    singleSGPerNodeScenario = sys.argv[1].lower() == "true"
    distributed = sys.argv[2].lower() == "true"
    namespace = sys.argv[3].lower()

    print(f"Watching the {namespace} namespace")
    global NAMESPACE
    NAMESPACE = namespace
    ...
    # Running non-distributed.
        ...
        ClusterState().initialize(PNS_scenario=singleSGPerNodeScenario, namespace=NAMESPACE)
        print(ClusterState())
...
def watch_services():
    ...
    """
    Watcher(PNS_scenario=True, namespace=NAMESPACE).watch_services()
