...
class SecurityGroup:
    ...
    def attach_to_node(self, n: Node):
        if not n:
            raise Exception(f"node {n} cannot be None!")
        self.attached_nodes.add(n)

    def detach_from_node(self, n: Node):
        if n not in self.attached_nodes:
            raise Exception(f"SG {self.name} ({self.id}) not attached to a Node!")
    ...
    def get_attached_nodes_string(self):
        ...
        for node in self.attached_nodes:
            attached_nodes_str += node.name + ", "
        return attached_nodes_str
