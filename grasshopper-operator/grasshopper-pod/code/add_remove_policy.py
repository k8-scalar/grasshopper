...
def add_policy(pol: Policy) -> None:
    ...
    if not isinstance(pol.allow[0][0], CIDR):
        if pol.allow[0][0] not in mapping:
            mapping[pol.allow[0][0]] = MapEntry()
        mapping[pol.allow[0][0]].allow_pols.append(pol)


def remove_policy(pol: Policy) -> None:
    mapping = ClusterState().get_map()
    s = mapping[pol.sel]
    a = mapping[pol.allow[0][0]]
