...
class SecurityGroupModule(ABC):
    ...
    def add_rule_to_remotes(SG: SecurityGroup, rule: Rule) -> None:
        ...
        try:
            ...
            logger.info(f"SGMod: Failed creating rule: {e}")
        ...
        if rule.id is None:
            ...
            for r in existing_rules:
                if r.target == rule.target and r.traffic == rule.traffic:
                    ...
                    break
            if rule.id is None:
                logger.info(f"SGMod: {rule} not found in {SG.name}")
                logger.info("Existing rules are:")
                for rule in SG.remotes:
                    logger.info(rule)
                return
        logger.info(f"SGMod: Removing rule {rule.id} from {SG.name}")
        neutron = OpenStackClient().get_neutron()
...
class SecurityGroupModulePNS(SecurityGroupModule):
    ...
    def SG_add_conn(pol: Policy, n: Node, m: Node) -> None:
        if n == m:
            logger.info(f"SGMod: Cannot add connection from {n.name} to itself in PNS mode.")
            return
        logger.info(f"SGMod: Adding connection from {n.name} to {m.name}")
    ...
    def SG_remove_conn(pol: Policy, n: Node, m: Node) -> None:
        logger.info(f"SGMod: Removing connection from {n.name} to {m.name}")
        if not isinstance(pol.allow[0][0], CIDR):
            ...
            logger.info(f"SGMod: removed rule from {SecurityGroupModulePNS.SGn(n).name}")
...
class SecurityGroupModulePLS(SecurityGroupModule):
    ...
    def remove_sg(L: LabelSet):
        ...
        logger.info(f"Removing SG: {SecurityGroupModulePLS.SGn(L)}")
        ...
        if SecurityGroupModulePLS.SGn(L) in ClusterState.get_security_groups().keys():
            ...
            logger.info(f"SGMod: Removing Security Group: {SecurityGroupModulePLS.SGn(L)}")
    ...
    def attach_security_group_to_node(sg: SecurityGroup, node: Node):
        ...
        if sg.name in attached_sgs:
            logger.info(f"{sg.name} already attached to {node.name}")
            return

        security_group_name = sg.name
        sg_id = sg.id
        logger.info(f"Attaching security group {security_group_name} to instance {node.name}")
    ...
    def detach_security_group(sg: SecurityGroup, node: Node):
        ...
        logger.info(f"Detaching security group {security_group_name} to instance {node.name}")
    ...
    def delete_security_group(sg_name):
        ...
        if not sg:
            logger.info(f"Security group '{sg_name}' not found.")
            return
        ...
        try:
            ...
            logger.info(f"Security group '{sg_name}' deleted successfully.")
        except Exception as e:
            logger.info(f"Failed to delete security group '{sg_name}': {e}")  
    ...
    def add_rule_to_remotes(sg: SecurityGroup, rule: Rule):
        ...
        try:
            ...
            logger.info(f"Cannot add rule to remotes: {e}")
