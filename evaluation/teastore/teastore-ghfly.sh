...
if [ "$pernode_input" == "y" ] || [ "$pernode_input" == "Y" ]; then
    logfile=${path_variable}/results/per-node/teastore-ghfly
    command="python3 ${path_variable}/ostackfiles/deleteRulesManually.py"
else
    logfile=${path_variable}/results/per-labelSet/teastore-ghfly
    command=""
fi

python3 grasshopper/ostackfiles/detach_defaultSG.py #attach default SG to workers if not attached
