...
if [ "$pernode_input" == "y" ] || [ "$pernode_input" == "Y" ]; then
    logfile=${path_variable}/experiments/results/per-node/netperf-gh-v2.log
else
    logfile=${path_variable}/experiments/results/per-labelSet/netperf-gh-v2.log
fi
...
for ((i=1; i<=10; i++)); do
	...
	#for i in {1..5}; do   echo "Run $i:";   kubectl exec -n test -i -t $(kubectl get pod -n test -l "app=netperf-host" -o name | sed 's/pods\///') --     netperf -H $(kubectl get pods -n test -l app=netperf-client -o go-template="{{range .items}}{{.status.podIP}}{{\"\n\"}}{{end}}" --field-selector spec.nodeName=worker-1)  -T2,2   -t TCP_RR -l 30 -c -C -- -r 1,1 -o MIN_LATENCY,MEAN_LATENCY,MAX_LATENCY | tail -n 1; done | awk -F ',' 'NR>1{for(i=1;i<=NF;i++)sum[i]+=$i; print $0} END{printf "Average: "; for(i=1;i<=NF;i++)printf "%s%.2f", (i==1)?"":",", sum[i]/5; print ""}' >> $logfile
	#echo >> $logfile
