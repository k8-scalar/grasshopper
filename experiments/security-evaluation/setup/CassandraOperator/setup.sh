...
kubectl apply -f "${SCRIPT_DIR}/yamls/service_account.yaml"

echo "Applying roles..."
kubectl apply -f "${SCRIPT_DIR}/yamls/role.yaml"

echo "Applying role bindings..."
kubectl apply -f "${SCRIPT_DIR}/yamls/role_binding.yaml"
...
kubectl apply -f "${SCRIPT_DIR}/yamls/cassandra-operator-pod.yaml"
