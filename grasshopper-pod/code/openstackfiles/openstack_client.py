...
class OpenStackClient:
    ...
    def known_project_keys(cls) -> list[str]:
        """All project keys with configured credentials (OS_PROJECTS_JSON, plus
        the "default" fallback project if no entry overrides it)."""
    ...
    def _load_credentials(cls) -> dict[str, dict]:
        ...
        # Legacy single-project fallback: flat OS_* env vars, as the "default"
        # project - only used if OS_PROJECTS_JSON didn't already define one.
        if DEFAULT_PROJECT_KEY not in credentials_by_key:
